    ...
    <div class="col-md-8">
        ...
        <div class="box box-primary">
            ...
            <form role="form" action="{{route('product.update',$product)}}" method="POST" enctype="multipart/form-data">
                ...
                <button type="button" onclick="addVariant()">add variants</button><br><br> --}}
                <div class="form-group">
                    <img src="{{ asset('storage/images/' . $product->image) }}" width="100" alt=""><br>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="slug" class="form-control" id="exampleInputEmail1"
                        placeholder="Enter Name Product" value="{{ $product->slug }}">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">Origin</label>
                    <input type="text" name="origin" class="form-control" id="exampleInputEmail1"
                        placeholder="Enter Origin" value="{{ $product->origin }}">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">Select Category</label>
                    <select name="category_id" class="form-control">
                        @foreach ($cate as $value)
                            <option value="{{ $value->id }}" {{ $product->CategoryID == $value->id ? 'selected' : '' }}>
                                {{ $value->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <textarea name="sortdescription" id="" cols="30" rows="5">{{ $product->sortdescription }}</textarea>
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">Description</label><br>
                    <textarea name="description" id="editor1" cols="80" rows="7">{{ $product->description }}</textarea>
                </div>
        </div>
        <!-- /.box-body -->
